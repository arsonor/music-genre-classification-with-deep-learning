...
class TestPrometheusUtility:
    ...
    def test_prometheus_safe_complex_names(self):
        ...
        assert prometheus_safe("F1-Score@Model.Performance") == "f1_score_model_performance"
...
class TestMonitoringEndpoint:
    ...
    def sample_current_data(self):
        ...
        data = {
            **{f"mfcc_{i}": np.random.randn(n_samples) + 0.1 for i in range(1, 14)},  # Slight shift
            "predicted_genre": np.random.choice(
                ["blues", "classical", "country", "disco", "hiphop"], 
                n_samples
            ),
    ...
    def test_metrics_endpoint_missing_reference_file(self, client):
        ...
        with patch('run_monitoring.os.path.exists') as mock_exists:
            mock_exists.side_effect = lambda path: path != "data/reference.parquet"
    ...
    def test_metrics_endpoint_missing_current_file(self, client):
        ...
        with patch('run_monitoring.os.path.exists') as mock_exists:
            mock_exists.side_effect = lambda path: path != "data/current.parquet"
    ...
    def test_metrics_endpoint_successful_response(self, mock_exists, mock_read_parquet, 
                                                 client, sample_reference_data, sample_current_data):
        ...
        with patch('run_monitoring.Dataset') as mock_dataset, \
             patch('run_monitoring.Report') as mock_report:
            ...
            mock_results = {
                "metrics": [
                    ...
                    {"metric_id": "drift_score", "value": {"mfcc_1": 0.15, "mfcc_2": 0.23}},
                    {"metric_id": "category_count", "value": {"blues": 20, "classical": 15}}
                ]
            }
            ...
            assert "# HELP evidently_metric Evidently monitoring metric" in response_text
    ...
    @patch('run_monitoring.pd.read_parquet')
    @patch('run_monitoring.os.path.exists')
    def test_metrics_endpoint_prometheus_format_validation(self, mock_exists, mock_read_parquet, 
                                                          client, sample_reference_data, sample_current_data):
    ...
    @patch('run_monitoring.pd.read_parquet')
    @patch('run_monitoring.os.path.exists')
    def test_metrics_endpoint_handles_non_numeric_values(self, mock_exists, mock_read_parquet, 
                                                        client, sample_reference_data, sample_current_data):
