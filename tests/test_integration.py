...
class TestEndToEndIntegration:
    ...
    def test_complete_prediction_flow(
        ...
        mock_audio_data = np.random.randn(66150).astype(
            np.float32
        ...
        mock_predictions = np.array(
            [[0.1, 0.05, 0.15, 0.02, 0.03, 0.9, 0.12, 0.03, 0.06, 0.04]]
        )
        ...
        response = client.post(
            "/predict",
            data={"file": (test_file, "test.wav"), "actual_genre": "jazz"},
    ...
    def test_monitoring_data_persistence(
        ...
        response = client.post(
            "/predict",
            data={"file": (test_file, "test.wav"), "actual_genre": "blues"},
            content_type="multipart/form-data",
        )
        ...
        combined_data.to_parquet.assert_called_once_with(
