...
class TestDataUtils:
    ...
    def test_prepare_dataset_custom_splits(self, mock_load):
        ...
        X_train, _y_train, X_val, _y_val, X_test, _y_test = prepare_dataset(
    ...
    def test_plot_history(self, mock_subplots, mock_savefig):
        ...
        plot_result = plot_history(mock_history, "test_plot.png")
        ...
        mock_ax1.plot.assert_any_call(
            [0.65, 0.75, 0.82, 0.88], label="Validation Accuracy"
        )
        ...
        assert plot_result == mock_fig


class TestModel:
    """Test cases for the model building function."""

    @patch("keras.Sequential")
    def test_build_model_structure(self, mock_sequential):
        ...
        model_result = build_model(input_shape, l2_reg, learning_rate)
...
"""
    ...
    def test_train_and_log_model(
        self, mock_plot, mock_build_model, mock_prepare, mock_mlflow
    ):
        ...
        mock_model = Mock()
        mock_history = Mock()
        mock_history.history = {"val_accuracy": [0.7, 0.8, 0.85, 0.9]}
    ...
    def test_train_and_log_model_missing_config(self, mock_open_func):
...
class TestBasicFunctionality:
    ...
    def test_data_validation_logic(self, mock_exists):
        ...
        validation_result = download_and_validate_data()

        assert validation_result.endswith("data_10.npz")
        assert os.path.isabs(validation_result)
...
class TestPipelineIntegration:
    ...
    def test_features_to_data_pipeline(self, mock_exists, mock_load):
        ...
        # pylint: disable=import-outside-toplevel
        from classifier.utils.data import prepare_dataset
        from classifier.pipeline.features import (
            extract_features,
            download_and_validate_data,
        )
        ...
        X_train, _y_train, X_val, _y_val, X_test, _y_test = prepare_dataset(data_path)
    ...
    def test_model_data_compatibility(self):
        ...
        with patch("classifier.utils.data.load_data") as mock_load:
            ...
            X_train, y_train, _X_val, _y_val, _X_test, _y_test = prepare_dataset(
                "/path/to/data.npz"
            )
